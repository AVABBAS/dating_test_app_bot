...
const ALLOWED_ACTIONS = new Set(["like", "pass", "superlike"]);
...
async function attachTelegramUser(req, res, next) {
  ...
  if (req.path === "/api/action" && req.method === "POST" && !ALLOWED_ACTIONS.has(String(req.body?.action || ""))) {
    return res.status(400).json({ error: "Invalid action. Expected like, pass, or superlike" });
  }
  ...
  if (!req.tg && process.env.NODE_ENV !== "production" && process.env.REQUIRE_TELEGRAM_AUTH !== "1") {
    ...
    if (devTgId) await loadTelegramUser(req, devTgId, { id: devTgId, first_name: "Dev" }, false, true);
  }

  // These handlers historically accepted telegramId from the query/body. When
  // it is omitted, bind it to the cryptographically verified Telegram identity
  // so the handler can never fall back to an attacker-controlled identity.
  if (req.tg?.verified && req.tg.id) {
    if (req.method === "GET" && (req.path.startsWith("/api/messages/") || req.path.startsWith("/api/chat-info/") || req.path.startsWith("/api/typing/"))) {
      if (!req.query.telegramId) req.query.telegramId = req.tg.id;
    }
    if (req.method === "POST" && req.path.startsWith("/api/typing/")) {
      if (!req.body.telegramId) req.body.telegramId = req.tg.id;
    }
  }
