...
// Telegram mini-app links may also be launched with a versioned path.
// Keep the same no-cache HTML shell for that path instead of relying on a
// physical /mini/<version> directory in Vite's output.
app.get('/mini/:version', (req, res) => {
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
  res.setHeader('Pragma', 'no-cache');
  res.setHeader('Expires', '0');
  res.sendFile(path.join(__dirname, 'frontend/dist/index.html'), {
