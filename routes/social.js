  ...
  try {
    ...
    res.json({
      ...
      events: events.map((e) => ({
        ...
        category: e.category, city: e.city, location: e.location, startsAt: e.startsAt,
...
router.get("/events/:id", async (req, res) => {
  ...
  try {
    ...
    if (req.query.telegramId) {
      ...
      if (me) {
        const membership = await prisma.eventAttendee.findUnique({ where: { eventId_userId: { eventId: id, userId: me.id } }, select: { id: true } });
        joined = Boolean(membership);
      }
    }
    res.json({ event: { ...event, host: event.host ? withComputedOnline(event.host) : null, attendeeCount: event._count.attendees, attendees: event.attendees.map((a) => withComputedOnline(a.user)), joined } });
  } catch (e) { console.error(e); res.status(500).json({ error: "Server error" }); }
});

router.post("/events/:id/join", authorizeTelegramId((req) => req.body.telegramId), async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid event id" });
  try {
    const user = await prisma.user.findUnique({ where: { telegramId: req.body.telegramId.toString() }, select: { id: true } });
    if (!user) return res.status(404).json({ error: "User not found" });
    await prisma.eventAttendee.upsert({ where: { eventId_userId: { eventId: id, userId: user.id } }, update: {}, create: { eventId: id, userId: user.id } });
    const count = await prisma.eventAttendee.count({ where: { eventId: id } });
    res.json({ ok: true, joined: true, attendeeCount: count });
  } catch (e) { console.error(e); res.status(500).json({ error: "Server error" }); }
});

router.post("/events/:id/leave", authorizeTelegramId((req) => req.body.telegramId), async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid event id" });
  try {
    const user = await prisma.user.findUnique({ where: { telegramId: req.body.telegramId.toString() }, select: { id: true } });
...
router.get("/leaderboard", async (req, res) => {
  try {
    ...
    const grouped = await prisma.like.groupBy({
      by: ["toUserId"], where: { action: { in: ["like", "superlike"] }, createdAt: { gte: since } },
      _count: { toUserId: true }, orderBy: { _count: { toUserId: "desc" } }, take: 20,
    });
    const ids = grouped.map((g) => g.toUserId);
    if (!ids.length) return res.json({ leaders: [] });
    const users = await prisma.user.findMany({ where: { id: { in: ids }, incognito: false }, select: publicUserSelect });
    const byId = new Map(users.map((u) => [u.id, u]));
    const leaders = grouped.filter((g) => byId.has(g.toUserId)).map((g, i) => ({ rank: i + 1, likes: g._count.toUserId, user: withComputedOnline(byId.get(g.toUserId)) }));
    res.json({ leaders });
  } catch (e) { console.error(e); res.status(500).json({ error: "Server error" }); }
});

// ────────────────────────────── Passport ──────────────────────────────
router.put("/passport/:telegramId", authorizeTelegramId((req) => req.params.telegramId), async (req, res) => {
  ...
  try {
    ...
    const user = await prisma.user.update({ where: { telegramId: req.params.telegramId.toString() }, data, select: { city: true, passportCity: true, latitude: true, longitude: true } });
    res.json({ ok: true, passport: user });
  } catch (e) { console.error(e); res.status(500).json({ error: "Server error" }); }
});

router.get("/passport/cities", (req, res) => {
  res.json({ cities: ["تهران", "مشهد", "اصفهان", "شیراز", "تبریز", "کرج", "اهواز", "قم", "کرمانشاه", "رشت", "یزد", "کیش"] });
});

// ──────────────────────── Notifications center ────────────────────────
router.get("/notifications/:telegramId", authorizeTelegramId((req) => req.params.telegramId), async (req, res) => {
  try {
...
router.post("/notifications/:telegramId/read", authorizeTelegramId((req) => req.params.telegramId), async (req, res) => {
  try {
    const user = await prisma.user.findUnique({ where: { telegramId: req.params.telegramId.toString() }, select: { id: true } });
    if (!user) return res.status(404).json({ error: "User not found" });
    if (req.body.id) {
...
// Keep unknown /api/* requests JSON even though server.js has an SPA catch-all.
