...
const TopPicks = ({ user }) => {
  ...
  const act = (u, action) => {
    setActing(true);
    api.action(user.telegramId, u.id, action)
      .then((r) => {
    ...
    <div className="lp-page">
      ...
      {picks.length === 0 ? (
        ...
        <div className="tp-grid">
          {picks.map((u) => (
            <button type="button" key={u.id} className="tp-card" onClick={() => setActive(u)}>
      ...
      {active && (
        <div className="explore-modal-overlay" onClick={() => setActive(null)}>
          <div className="explore-modal" onClick={(e) => e.stopPropagation()}>
            <button type="button" className="explore-modal-close" onClick={() => setActive(null)}><X size={18} /></button>
            ...
            <div className="explore-modal-actions">
              <button type="button" className="action-btn btn-pass" disabled={acting} onClick={() => act(active, 'pass')}><X size={26} /></button>
              <button type="button" className="action-btn btn-super" disabled={acting} onClick={() => act(active, 'superlike')}><Star size={22} /></button>
              <button type="button" className="action-btn btn-like" disabled={acting} onClick={() => act(active, 'like')}><Heart size={26} fill="#fff" /></button>
            </div>
          </div>
        </div>
      )}
